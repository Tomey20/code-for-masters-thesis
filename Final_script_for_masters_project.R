...
combined_plot
##############
#Visualise chromVAR bulk, supplementary figure
##############
chromVAR_object <- readRDS("/work/Home/Data_for_masters_project/chromVAR_bulk_4_replicates_all_peaks")
chromVAR_object <- chromVAR_object@assays$chromvar$data
chromVAR_object <- as.matrix(chromVAR_object)
# Load required library
library(Matrix)
# Convert all entries to numeric
activities <- matrix(
  as.numeric(chromVAR_object),
  nrow = nrow(chromVAR_object),
  ncol = ncol(chromVAR_object),
  dimnames = dimnames(chromVAR_object)  # preserve row/column names
)
# Extract base column names (removing suffixes like .1, .2, etc.)
base_colnames <- sub("\\.\\d+$", "", colnames(activities))
...
Heatmap(
  ...
  column_names_gp   = gpar(fontsize = BASE_FONTSIZE,   fontface = "bold"),
  row_names_gp      = gpar(fontsize = BASE_FONTSIZE-2),
  column_title      = "ChromVAR Motif Activity Heatmap\n(4 Replicates, All Peaks)",
  row_title         = "Transcription Factors",
  column_title_gp   = gpar(fontsize = BASE_FONTSIZE+4, fontface = "bold"),
  row_title_gp      = gpar(fontsize = BASE_FONTSIZE+4, fontface = "bold"),
  heatmap_legend_param = list(
...
###############
# Run and Visualise IMAGE DA without GC adjusted content, supplementary figure
###############
library(BSgenome.Hsapiens.UCSC.hg38)
library(GenomicRanges)
library(Biostrings)
...
library(EDASeq)
ATAC <- readRDS("/work/Home/Data_for_masters_project/ATAC_summarizedExperimentObject_TFatlas_bulk_for_seurat_clusters_4_replicates_all peaks")
normalizeEnhancers <- function(ATAC, normalize = TRUE, standardize  = TRUE) { 

  sample_names  <- colnames(ATAC)
  cell_types    <- gsub("\\..*", "", sample_names)          
  replicate_ids <- ave(cell_types, cell_types, FUN = seq)
  
  sample_conditions <- data.frame(
    sample_id = sample_names,
    cell_type = factor(cell_types,   levels = unique(cell_types)),
    replicate = factor(replicate_ids)
  )
  rownames(sample_conditions) <- sample_conditions$sample_id
  
  counts_mat <- as.matrix(
    SummarizedExperiment::assays(ATAC, withDimnames = FALSE)$Peaks
  )
  
  dge <- edgeR::DGEList(counts = counts_mat,
                        group  = sample_conditions$cell_type)
  ...
  qlf  <- edgeR::glmQLFTest(fit, coef = 2:ncol(design))
  
  da_tbl <- data.frame(
    peak   = rownames(qlf$table),
    logFC  = rowMeans(qlf$table[ , grep("^logFC.cell_type", colnames(qlf$table)) ],
                      na.rm = TRUE),
  ...
  sig_peaks <- subset(da_tbl, FDR < 0.05)$peak
  counts_da <- counts_mat[rownames(counts_mat) %in% sig_peaks, ]
  
  if (normalize) {
    counts_da <- t( t(counts_da) / (colSums(counts_da) / 1e7) ) 
  }
  if (standardize) {
    col_means <- colMeans(counts_da)
    col_sds   <- apply(counts_da, 2, sd)
    counts_da <- t( (t(counts_da) - col_means) / col_sds )
  }
...
start_time3 <- Sys.time()
normEnhancers <- normalizeEnhancers(ATAC, normalize = TRUE, standardize = TRUE)
end_time3 <- Sys.time()
time_normEnhancers <- end_time3-start_time3
...
end_time4 <- Sys.time()
time_activities <- end_time4-start_time4
#saveRDS(activities, "/work/Home/Data_for_masters_project/activities_chromVAR_motifs_edgeR_DA_not_GC_adjusted")
activities <- readRDS("/work/Home/Data_for_masters_project/activities_chromVAR_motifs_edgeR_DA_not_GC_adjusted")
activities <- as.matrix(activities)
# Load required library
library(Matrix)
# Convert all entries to numeric
...
IMAGE_DA_peaks <- Heatmap(
  ...
  column_names_gp   = gpar(fontsize = BASE_FONTSIZE,   fontface = "bold"),
  row_names_gp      = gpar(fontsize = BASE_FONTSIZE-2),
  column_title      = "IMAGE Enhancer Motif Activity Heatmap\n(4 Replicates, Differential accecible peaks, Non-GC Adjusted)",
  row_title         = "Transcription Factors",
  column_title_gp   = gpar(fontsize = BASE_FONTSIZE+4, fontface = "bold"),
  row_title_gp      = gpar(fontsize = BASE_FONTSIZE+4, fontface = "bold"),
  heatmap_legend_param = list(
...
IMAGE_DA_peaks <- grid.grabExpr(draw(IMAGE_DA_peaks))

# load chromVAR single cell
chromVAR_object <- readRDS("/work/Home/Data_for_masters_project/chromVAR_results/chromVAR_single_cell_all_peaks.rds")
chromVAR_object <- chromVAR_object@assays$chromvar$data
# Load mapping of names
library(chromVARmotifs)
...
expr_threshold <- 0.2
depl_threshold <- 1  # 1-fold => log2(1) = 0   , 2-fold => log2(2) = 1
clusters <- colnames(enrichment_results)
df_all <- list()
k <- 1
# loop through the fold changes
for (fold in all_folds) {
...
library(patchwork)
heatmap_patch <- wrap_elements(full = IMAGE_DA_peaks)
combined_plot <- (heatmap_patch) / (p) +
...
combined_plot
##################
# Run and visualise Fraction of cells where peaks are detected non-GC adjusted , supplementary figure
##################
library(BSgenome.Hsapiens.UCSC.hg38)
library(GenomicRanges)
library(Biostrings)
...
normalizeEnhancers = function(ATAC, normalize = TRUE, standardize = TRUE) {
  ...
  # Normalize
  if (normalize) {
    normalized_counts = t(t(counts_mat) / (colSums(counts_mat) / 10000000))
  }
  
  # Standardize
  if (standardize) {
    Cols <- colMeans(normalized_counts)
    SD <- apply(normalized_counts[,c(1:ncol(normalized_counts))],2,FUN="sd")
    normalized_counts <- t( ((t(normalized_counts) - Cols) / SD) )
  }
...
### Normalize Enhancers and read filtered motifs
normEnhancers <- normalizeEnhancers(ATAC, normalize = TRUE, standardize = TRUE)
filteredMotifs <- readRDS("/work/Home/Data_for_masters_project/filteredMotifs_chromVAR_motifs_4_replicates_GC_fraction_of_cells")

### Calculate Enhancer Motif Activity
activities <- calculateEnhancerMotifActivity(motifs = filteredMotifs, enhancers = normEnhancers, ncores = 2, a = 0)
#saveRDS(activities, "/work/Home/Data_for_masters_project/activities_chromVAR_motifs_4_replicates_fraction_of_cells_not_GC_adjusted")
activities <- readRDS("/work/Home/Data_for_masters_project/activities_chromVAR_motifs_4_replicates_fraction_of_cells_not_GC_adjusted")
...
IMAGE_fraction_peaks <- Heatmap(
  ...
  column_names_gp   = gpar(fontsize = BASE_FONTSIZE,   fontface = "bold"),
  row_names_gp      = gpar(fontsize = BASE_FONTSIZE-2),
  column_title      = "IMAGE Enhancer Motif Activity Heatmap\n(4 Replicates, ≥1% Cells with Peaks, Alpha = 0, Non-GC Adjusted)",
  row_title         = "Transcription Factors",
  column_title_gp   = gpar(fontsize = BASE_FONTSIZE+4, fontface = "bold"),
  row_title_gp      = gpar(fontsize = BASE_FONTSIZE+4, fontface = "bold"),
  heatmap_legend_param = list(
...
IMAGE_fraction_peaks <- grid.grabExpr(draw(IMAGE_fraction_peaks))
...
library(patchwork)
heatmap_patch <- wrap_elements(full = IMAGE_fraction_peaks)
combined_plot <- (heatmap_patch) / (p) +
  plot_layout(heights = c(2, 1)) +
  plot_annotation(tag_levels = "A") &  
  theme(
...
###################
# Experimenting with whitening, supplementary figure
###################
...
filteredMotifs_covariance <- cov(as.matrix(filteredMotifs))

# Regularize the covariance matrix by adding a small value to the diagonal
epsilon <- 1e-15  # Small positive constant to avoid NA's
filteredMotifs_covariance_reg <- filteredMotifs_covariance + diag(epsilon, nrow(filteredMotifs_covariance))

# Compute ZCA whitening matrix
W.ZCAcor <- whiteningMatrix(filteredMotifs_covariance_reg, method="ZCA-cor")
#saveRDS(W.ZCAcor, "/work/Home/Data_for_masters_project/Whitening_matrix_W")

# whitened data
Z = tcrossprod(as.matrix(filteredMotifs), W.ZCAcor) 
#saveRDS(Z, "/work/Home/Data_for_masters_project/Whitened_data_Z")

activities = calculateEnhancerMotifActivity(motifs = Z, enhancers = normEnhancers, ncores = 2, a = 0)
#saveRDS(activities, "/work/Home/Data_for_masters_project/activities_chromVAR_motifs_GC_and_edgeR_whitened")
activities <- readRDS("/work/Home/Data_for_masters_project/activities_chromVAR_motifs_GC_and_edgeR_whitened")
W.ZCAcor <- readRDS("/work/Home/Data_for_masters_project/Whitening_matrix_W")
filteredMotifs <- readRDS("/work/Home/Data_for_masters_project/filteredMotifs_chromVAR_motifs_4_replicates_all_peaks")

rownames(activities) <- colnames(filteredMotifs)
unwhitened_activities <- t(W.ZCAcor) %*% activities
# Assign proper row and column names
colnames(unwhitened_activities) <- colnames(activities)
rownames(unwhitened_activities) <- colnames(filteredMotifs)
activities <- as.matrix(unwhitened_activities)
# Load required library
library(Matrix)
# Convert all entries to numeric
activities <- matrix(
...
IMAGE_ZCA <- Heatmap(
  ...
  column_names_gp   = gpar(fontsize = BASE_FONTSIZE,   fontface = "bold"),
  row_names_gp      = gpar(fontsize = BASE_FONTSIZE-2),
  column_title      = "IMAGE Enhancer Motif Activity Heatmap\n(4 Replicates, Differential Accessible Regions, Alpha = 0, Whitened)",
  row_title         = "Transcription Factors",
  column_title_gp   = gpar(fontsize = BASE_FONTSIZE+4, fontface = "bold"),
  row_title_gp      = gpar(fontsize = BASE_FONTSIZE+4, fontface = "bold"),
  heatmap_legend_param = list(
...
IMAGE_ZCA <- grid.grabExpr(draw(IMAGE_ZCA))
...
library(patchwork)
heatmap_patch <- wrap_elements(full = IMAGE_ZCA)
combined_plot <- (heatmap_patch) / (p) +
  plot_layout(heights = c(2, 1)) +
  plot_annotation(tag_levels = "A") &  
  theme(
...
#############
# not double normalised DA enhancer motif activities
###############
ATAC <- readRDS("/work/Home/Data_for_masters_project/ATAC_summarizedExperimentObject_TFatlas_bulk_for_seurat_clusters_4_replicates_all peaks")
normalizeEnhancers = function(ATAC, normalize = TRUE, standardize = TRUE, genome) {
  #### create design matrix
  ...
  normalized_counts_fqfq <- normCounts(seq_obj_gc)
...
activities = calculateEnhancerMotifActivity(motifs = filteredMotifs, enhancers = normEnhancers, ncores = 2, a = 0)
#saveRDS(activities, "/work/Home/Data_for_masters_project/activities_chromVAR_motifs_not_double_normalisation_for_sequencing_depth")
activities <- readRDS("/work/Home/Data_for_masters_project/activities_chromVAR_motifs_not_double_normalisation_for_sequencing_depth")
activities <- as.matrix(activities)
# Load required library
library(Matrix)
# Convert all entries to numeric
...
IMAGE_FQ <- Heatmap(
  ...
  column_names_gp   = gpar(fontsize = BASE_FONTSIZE,   fontface = "bold"),
  row_names_gp      = gpar(fontsize = BASE_FONTSIZE-2),
  column_title      = "IMAGE Enhancer Motif Activity Heatmap\n(4 Replicates, Differential Accessible Regions, Alpha = 0, Only EDASeq::withinLaneNormalization)",
  row_title         = "Transcription Factors",
  column_title_gp   = gpar(fontsize = BASE_FONTSIZE+4, fontface = "bold"),
  row_title_gp      = gpar(fontsize = BASE_FONTSIZE+4, fontface = "bold"),
  heatmap_legend_param = list(
...
IMAGE_FQ <- grid.grabExpr(draw(IMAGE_FQ))
...
library(patchwork)
heatmap_patch <- wrap_elements(full = IMAGE_FQ)
combined_plot <- (heatmap_patch) / (p) +
  plot_layout(heights = c(2, 1)) +
  plot_annotation(tag_levels = "A") &  
  theme(
...
########
# not double normalised DA with whitening
#######
library(whitening)
ATAC <- readRDS("/work/Home/Data_for_masters_project/ATAC_summarizedExperimentObject_TFatlas_bulk_for_seurat_clusters_4_replicates_all peaks")
normalizeEnhancers = function(ATAC, normalize = TRUE, standardize = TRUE, genome) {
...
data("human_pwms_v2")
genome <- BSgenome.Hsapiens.UCSC.hg38
# Run norm enhancers with only within sample normalisation for GC
normEnhancers <- normalizeEnhancers(ATAC, normalize = TRUE, standardize = TRUE, genome = genome)
...
# Compute ZCA whitening matrix
W.ZCAcor <- whiteningMatrix(filteredMotifs_covariance_reg, method="ZCA-cor")
#saveRDS(W.ZCAcor, "/work/Home/Data_for_masters_project/Whitening_matrix_W_only_FQ")

# whitened data
Z = tcrossprod(as.matrix(filteredMotifs), W.ZCAcor) 
#saveRDS(Z, "/work/Home/Data_for_masters_project/Whitened_data_Z_only_FQ")

activities = calculateEnhancerMotifActivity(motifs = Z, enhancers = normEnhancers, ncores = 2, a = 0)
#saveRDS(activities, "/work/Home/Data_for_masters_project/activities_chromVAR_motifs_GC_and_edgeR_whitened_only_FQ")
activities <- readRDS("/work/Home/Data_for_masters_project/activities_chromVAR_motifs_GC_and_edgeR_whitened_only_FQ")
W.ZCAcor <- readRDS("/work/Home/Data_for_masters_project/Whitening_matrix_W_only_FQ")
filteredMotifs <- readRDS("/work/Home/Data_for_masters_project/filteredMotifs_chromVAR_motifs_4_replicates_all_peaks")
...
IMAGE_ZCA <- Heatmap(
  ...
  column_names_gp   = gpar(fontsize = BASE_FONTSIZE,   fontface = "bold"),
  row_names_gp      = gpar(fontsize = BASE_FONTSIZE-2),
  column_title      = "IMAGE Enhancer Motif Activity Heatmap\n(4 Replicates, Differential Accessible Regions, Alpha = 0, Whitened, Only EDASeq::withinLaneNormalization)",
  row_title         = "Transcription Factors",
  column_title_gp   = gpar(fontsize = BASE_FONTSIZE+4, fontface = "bold"),
  row_title_gp      = gpar(fontsize = BASE_FONTSIZE+4, fontface = "bold"),
  heatmap_legend_param = list(
